...
const RekapSkripsiDetail = () => {
  ...
  useEffect(() => {
    GetDetailSkripsiMahasiswa();
  }, []);

  const GetDetailSkripsiMahasiswa = async () => {
    try {
      const res = await Http.get(
        `/departemen/getsevenyears/Skripsi/detail/${angkatan}&${status}`,
        {
        ...
        <div className="flex-1 flex flex-col p-4 h-96">
          <div className="flex-1 flex flex-col p-4" ref={componentRef}>
            <h1 className="text-2xl text-center font-bold">
              {status === "Lulus"
                ? "Daftar Sudah Lulus Skripsi Mahasiswa Informatika"
                : "Daftar Belum Lulus Skripsi Mahasiswa Informatika"}
            </h1>
...
export default RekapSkripsiDetail;
